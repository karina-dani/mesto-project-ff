...
function renderUserInfo(info) {
  ...
  editProfileImageButton.setAttribute(
    "style",
...
function renderLoading(isLoading, form) {
  ...
  modalPreloader.classList.toggle("popup__preloader_visible", isLoading);
}
...
editProfileImageForm.addEventListener(
  "submit",
  handleEditProfileImageFormSubmit
);
...
//функция поиска карточки для удаления
function findDeletedCard(card, cardId) {
...
function deleteCard(cardId) {
  const cards = Array.from(document.querySelectorAll(".places__item"));

  cards.find((card) => {
    findDeletedCard(card, cardId);
  });
}
...
function likeCard(likeButton, likeCount, cardId) {
  const handleLikes = likeButton.classList.contains("card__like-button_is-active")
    ? deleteLikeFromUserCard
    : addLikeToUserCard;
  handleLikes(cardId)
    .then((res) => {
